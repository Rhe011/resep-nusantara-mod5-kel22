...
export default function RecipeDetail({ recipeId, onBack, onEdit, category = "makanan" }) {
  ...
  const handleShare = async () => {
    const url = `${window.location.origin}/resep/${category}/${recipeId}`;

    if (navigator.share) {
      try {
        await navigator.share({
          title: recipe?.name || "Resep",
  ...
  const handleSubmitReview = async (e) => {
    ...
    const user_identifier = userProfile.username || getUserIdentifier();
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <button
            onClick={onBack}
            className={`mt-4 px-4 py-2 bg-${colors.primary}-600 text-white rounded-lg hover:bg-${colors.primary}-700`}
          >
            Kembali
          </button>
        </div>
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br ${colors.gradient} pb-20 md:pb-8`}>
      ...
      <ConfirmModal
        ...
        title="Hapus Resep"
        message={`Apakah Anda yakin ingin menghapus resep "${recipe?.name}"?`}
        ...
        <div className="bg-white/60 backdrop-blur-sm rounded-3xl p-6 md:p-8 shadow-xl border border-white/40">
          <div className="flex items-center justify-between mb-6">
            ...
            <button
              ...
              className={`px-4 py-2 bg-${colors.primary}-600 text-white rounded-xl hover:bg-${colors.primary}-700 font-medium`}
            >
            ...
            >
              ...
              <div className="mb-4">
                ...
                <textarea
                  ...
                  rows={4}
                  className="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 bg-white resize-none"
                />
              </div>
              ...
              <button
                ...
                className={`w-full md:w-auto px-6 py-3 bg-${colors.primary}-600 text-white rounded-xl hover:bg-${colors.primary}-700 disabled:opacity-50 flex items-center gap-2`}
              >
